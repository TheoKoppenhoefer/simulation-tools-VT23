...
\end{wrapfigure}
 ...
 In particular our ODE is given by
\begin{align*}
...
\end{align*}
The plot of a numerical solution to this problem for $k=1$ can be seen in figures \ref{pl:StateTime1} and \ref{pl:PhasePortrait1}.
...
\end{align*}
We expect the approximate total energy to be constant which indeed can be seen in Figure \ref{pl:EnergyPlot1}.
Because of this property we can use the relative variation of the approximate total energy as an index meas implement
\begin{python}
import numpy as np
...
The graphs are presented in polar coordinates where $r$ refers to the length of the spring and $theta$ refers to the angle conformed between the pendulum and the vertical axis.

It can be observed that for a step size of $h=0.01$ Explicit Eurler \(Figure x\) already shows instability for values of $k=50$ while RK4 \(Figure x\) with that same step size remains stable for values up to $k=3000$.
...
\caption{\pyth{nfcns/nsteps} Explicit Euler $h=0.01$ $k=50$}
...
\caption{\pyth{njacs/nsteps} RK4 $h=0.01$ $k=3000$}
\label{pl:njacs_nsteps1}
\end{minipage}
\end{figure}
	
For Explicit Euler \(Figure x\), by keeping the value of $k$ constant and reducing the step size by a decimal place we can see how the instability is attenuated presenting a similar amplitude over time.
It takes a much larger step size and spring constant for RK4 to become unstable \(Figure x\), once unstabilized it’s growth is much more rapid than Explicit Euler’s and it does so without oscillating.
...
\caption{\pyth{nfcns/nsteps} Explicit Euler $h=0.001$ $k=50$}
\label{pl:nfcns_nsteps1}
\end{minipage}
\hfill
\begin{minipage}[b]{0.45\textwidth}
...
\caption{\pyth{njacs/nsteps} RK4 $h=0.1$ $k=975$}
\label{pl:njacs_nsteps1}
\end{minipage}
\end{figure}

It is interesting to observe that the oscillation of the spring is rapidly dumped when using RK4 \(Figure x\), a behavior similar to that presented by implicit methods.
...
\caption{\pyth{njacs/nsteps} RK4 $h=0.1$ $k=300$}
\label{pl:stability3}
\end{minipage}
\end{figure}
...
It is interesting to see how the oscillation of the spring decays for implicit methods. This decay can be attenuated by reducing the step size \(Figure x\) or accelerated by increasing the value of the spring constant \(Figure x\).
...
To better observe this decay \(Figure x & Figure x\) the domain is increased to $\[0, 100\]$.

%GRAPHHHHHHHHHHHHS

Hablar de la relacion entre decay y la region de estabilidad
...
In the first specific test of CVODE we solve our toy problem for increasing $k$. Hereby we switch between the BDF and Adam-Moultons discretisation method. We also vary the \pyth{maxorder} parameter for both methods.
A higher $k$ reflects a problem which is more stiff. As a stiff problem requires finer steps the number of steps \pyth{nsteps} increases as $k$ increases which is what can be seen in figure \ref{pl:nsteps1}. As the number of function evaluations per stepsize \pyth{nfcns/nsteps} hovers slightly above 1 for all methods (c.f.\ figure \ref{pl:nfcns_nsteps1}) the number of function evaluations increase correspondingly as can be seen in figure \ref{pl:nfcns1}. There is however a difference in how many steps each method needs. The BDF-method requires in general more steps than the Adams-Moulton method. And the general trend is that the number of steps increases as \pyth{maxord} is reduced. 
